var createError = require('http-errors');
var express = require('express');
var path = require('path');
var cookieParser = require('cookie-parser');
var logger = require('morgan');
var app = express();

// Middleware to parse JSON request bodies
app.use(express.json());

// Add a custom token to log the request body
logger.token('body', (req) => JSON.stringify(req.body));

// Use morgan to log requests
app.use(
  logger(':method :url :status :response-time ms - :res[content-length] :body', {
    skip: (req, res) => res.statusCode !== 400, // Log only when status code is 400
  })
);

// Example route to trigger a 400 error
app.post('/users', (req, res) => {
  if (!req.body.name || !req.body.company) {
    return res.status(400).json({ error: 'Missing name or company' });
  }
  res.json({ message: 'Success' });
});

var indexRouter = require('./routes/index');
var usersRouter = require('./routes/users');
var bookRouter = require('./routes/book'); // Import the book router


var app = express();

// view engine setup
app.set('views', path.join(__dirname, 'views'));
app.set('view engine', 'pug');

app.use(logger('dev'));
app.use(express.json());
app.use(express.urlencoded({ extended: false }));
app.use(cookieParser());
app.use(express.static(path.join(__dirname, 'public')));



app.use('/books', bookRouter); // Mount the book router at the /books path
app.use('/', indexRouter);
app.use('/users', usersRouter);

// catch 404 and forward to error handler
app.use(function(req, res, next) {
  next(createError(404));
});

// error handler
app.use(function(err, req, res, next) {
  // set locals, only providing error in development
  res.locals.message = err.message;
  res.locals.error = req.app.get('env') === 'development' ? err : {};

  // render the error page
  res.status(err.status || 500);
  res.render('error');
});

module.exports = app;
